    ...
    <div className={reverse ? "animate-marquee-rev flex whitespace-nowrap" : "animate-marquee flex whitespace-nowrap"}>
      {items.map((item, idx) => (
        ...
        >
          ...
          <span className="rounded-full bg-amber-50 px-2 py-0.5 text-[10px] text-amber-700">{item.category}</span>
        </div>
      ))}
    </div>
  );
    ...
    <section id="models" className="overflow-hidden bg-[var(--bg-subtle)] py-20">
      <SectionWrapper>
        <div className="mb-10 text-center">
          <span className="text-[12px] font-semibold uppercase tracking-[0.06em] text-[var(--text-muted)]">接入的模型</span>
          <h2 className="font-display mt-3 text-[32px] font-bold">一个 Key，调用全球所有主流模型</h2>
        </div>
      </SectionWrapper>
